...
namespace NodeCanvas.Tasks.Actions
{
    ...
    public class WanderInSideArea : ActionTask<NavMeshAgent>
    {
        ...
        [RequiredField]
        public BBParameter<GameObject> centerArea;
        ...
        protected override void OnUpdate() {
            if ( !agent.pathPending && agent.isOnNavMesh && agent.remainingDistance <= agent.stoppingDistance + keepDistance.value ) {
                if ( repeat ) {
                    DoWander();
                } else {
                    EndAction();
        ...
        void DoWander() {
            ...
            while ( ( wanderPos - agent.transform.position ).sqrMagnitude < ( min * min ) ) {
                wanderPos = ( Random.insideUnitSphere * max ) + centerArea.value.transform.position;
            }

            NavMeshHit hit;
            if ( NavMesh.SamplePosition(wanderPos, out hit, float.PositiveInfinity, NavMesh.AllAreas) && agent.isOnNavMesh) {
